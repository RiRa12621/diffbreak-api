...
func main() {
	...
	http.Handle("/detect", pkg.WithCORS(pkg.WrapHandler("detect", pkg.DetectHandler(client, logger), logger)))
	http.Handle("/analyze", pkg.WithCORS(pkg.WrapHandler("analyze", pkg.AnalyzeHandler(client, *llmPtr, logger), logger)))
